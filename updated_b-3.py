...
class DownwardDockingNode(Node):
    ...
    def send_manual_control(self, vx, vy, vz, vyaw):
        if self.master:
            ...
            self.master.mav.manual_control_send(
                self.master.target_system, rx, ry, rz, rr, 0
            )
    ...
    def main_loop(self):
        ...
        if ids is not None:
            ...
            for i in range(len(ids)):
                ...
                if m_id in TARGET_IDS:
                    ...
                    # Bulunan hedefin 4 köşesini 3D olarak tanımlıyoruz
                    marker_3d_corners = [
        ...
        if self.num_detected > 0:
            ...
            if success and tvec[2][0] > 0:
                # 3D Merkezi (0,0,0) 2D Ekrana yansıt
    ...
    def draw_hud(self, frame, deadzone_px):
        ...
        cv2.putText(frame,
            f"MODE: {self.state.name} | DIST: {self.current_dist:.2f}m | MARKERS: {self.num_detected}",
