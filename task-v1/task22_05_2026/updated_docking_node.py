...
class DockingNode(Node):
    ...
    def send(self, vx, vy, vz, yaw):
        ...
        try:
            ...
            # [Risk 10] Seri bağlantı kopması veya MAVLink hatası yakalanır
            self.get_logger().error(f"MAVLink gönderim hatası: {e}")

    def loop(self):
        now = time.time()
        dt = min(now - self.last_loop_t, 0.1)
        self.last_loop_t = now
        if not self.armed:                      # ← guard eklendi
            self.get_logger().warn("Araç ARM değil, döngü atlandı.")
            return

        ret, frame = self.cap.read()
        if not ret:
            self.send(0.0, 0.0, 0.0, 0.0)  # veya last komutları
            return
