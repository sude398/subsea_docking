...
# MAVROS OverrideRCIn imported exactly as specified
...
# ── ANA DÜĞÜM ─────────────────────────────────────────────────────────────────
class DownwardDockingNodeAndCamera(Node):
    def __init__(self):
        super().__init__('downward_docking_and_camera_node')
        ...
        # CAMERA PID & CONFIG PARAMETERS
        self.declare_parameter('cam_tilt_search', 1750) # Look as far down as possible without seeing chassis
        self.declare_parameter('cam_kp', 0.1) # Dynamic camera tracking speed
        self.declare_parameter('cam_tilt_min', 1100) # Max up
        self.declare_parameter('cam_tilt_max', 1800) # Max down (clipped to avoid chassis intrusion)
        ...
        # Camera internal tilt PWM state
        self.cam_tilt_pwm = 1500.0 
        ...
        self.get_logger().info(f"⚓ Downward Node & Camera Tracker: Kamera {self.video_source} üzerinden başlatıldı.")
    ...
    def publish_camera_tilt(self):
        """Sends the OverrideRCIn message based on self.cam_tilt_pwm"""
        msg = OverrideRCIn()
        channels = [0] * 18
        # Index 8 (Channel 9) is targeted for tilt logic
        channels[8] = int(self.cam_tilt_pwm)
    ...
    def main_loop(self):
        ...
        cam_tilt_search = self.get_parameter('cam_tilt_search').value
        cam_kp = self.get_parameter('cam_kp').value
        cam_tilt_min = self.get_parameter('cam_tilt_min').value
        cam_tilt_max = self.get_parameter('cam_tilt_max').value
        ...
        # --- KONTROL MANTIĞI & KAMERA HAREKET ALGORİTMASI ---
        ...
        if self.locked:
            ...
            # DYNAMIC CAMERA ALGORITHM: Track ArUco marker directly
            # If marker is above image center (err_y is negative), decrease PWM to tilt UP.
            # If marker is below image center (err_y is positive), increase PWM to tilt DOWN.
            if abs(err_y) > deadzone_px:
                self.cam_tilt_pwm += (err_y * cam_kp)
                self.cam_tilt_pwm = np.clip(self.cam_tilt_pwm, cam_tilt_min, cam_tilt_max)
            ...
            if self.armed:
                ...
                if time_since_lost < lost_timeout:
                    ...
                    vx, vy, vz, vyaw = 0.0, 0.0, 0.0, 0.0
                    # Hold current camera tilt during timeout failsafe
                else:
                    self.state = ROVState.SEARCHING
                    vz = vz_search
                    # DYNAMIC CAMERA ALGORITHM: Reset to DOWN target when searching
                    diff = cam_tilt_search - self.cam_tilt_pwm
                    if abs(diff) > 5.0:
                        self.cam_tilt_pwm += 5.0 * (1.0 if diff > 0 else -1.0) # Smoothly approach search tilt
                    else:
                        self.cam_tilt_pwm = cam_tilt_search

        # Eşzamanlı Komut Gönderimi
        self.send_manual_control(vx, vy, vz, vyaw)
        self.publish_camera_tilt()
    ...
    def draw_hud(self, frame, deadzone_px):
        ...
        # Typecasting int explicitly for cv2
        ...
        # Displaying dynamically computed Camera Tilt
        cv2.putText(frame, f"CAM TILT: {int(self.cam_tilt_pwm)} PWM", (50, 100), 0, 1, (255, 200, 0), 2)
...
def main():
    rclpy.init()
    node = DownwardDockingNodeAndCamera()
    try:
        rclpy.spin(node)
